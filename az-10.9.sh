...
sql = "select %s, item.ztitle, sum((item.zenddate-item.zstartdate)/3600.0), item.zlocation, item.znotes " % startdate_expr
sql += "from zcalendaritem as item inner join znode as cal on item.zcalendar = cal.z_pk "
sql += "where cal.ztitle = '%s' " % cal_name
sql += "and  item.zstartdate >= %d " % startdate
sql += "group by item.ztitle, item.zlocation, item.znotes, %s " % startdate_expr
sql += "order by %s, item.zlocation, item.ztitle, item.znotes" % startdate_sort
