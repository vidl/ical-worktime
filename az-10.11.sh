...
sql = "select %s, item.ztitle, sum((item.zenddate-item.zstartdate)/3600.0), loc.ztitle, item.znotes " % startdate_expr
sql += "from zcalendaritem as item "
sql += "inner join znode as cal on item.zcalendar = cal.z_pk "
sql += "inner join zlocation as loc on item.zstructuredlocation = loc.z_pk "
sql += "where cal.ztitle = '%s' " % cal_name
sql += "and  item.zstartdate >= %d " % startdate
sql += "group by item.ztitle, loc.ztitle, item.znotes, %s " % startdate_expr
sql += "order by %s, loc.ztitle, item.ztitle, item.znotes" % startdate_sort
